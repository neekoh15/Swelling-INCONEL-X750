...
S = []
T = []
with open('CS vs T (all fits)/347-1-raw.txt') as file:
...
vol_after_swelling = sum([p.vol_after2 for p in puntos])
...
with open('puntos.txt', 'a') as p:
    for punto in puntos:
        p.write(f'{punto.x}, {punto.y}, {punto.z}, {punto.T}, {punto.vol}, {punto.vol_after2}')
